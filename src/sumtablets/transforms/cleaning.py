"""Remove editorial markup (brackets, restorations, glosses) from transliterations."""
...
def clean(df: pd.DataFrame) -> pd.DataFrame:
    """Remove editorial markup from transliterations.

    Adds a 'transliteration_clean' column, then filters out tablets whose
    cleaned transliteration is empty (only special tokens).

    Args:
        df: DataFrame with 'id' and 'transliteration' columns.

    Returns:
        DataFrame with added 'transliteration_clean' column.
    """
    df = df.copy()
